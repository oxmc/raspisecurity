...
Scriptdir="/home/pi/Raspisecurity/scripts"
Confdir="/home/pi/Raspisecurity/configs"

#Clear screen
clear

#Get user input
echo ""press enter to not change a value.
read -p "Start in daemon (background) mode and release terminal (default: off) : " dmon
read -p "Start in Setup-Mode, daemon disabled. (default: off) : " stupm
#read -p "Videodevice to be used for capturing  (default /dev/video0) for FreeBSD default is /dev/bktr0 : " vdodv
#read -p "The video input to be used (default: -1) Should normally be set to 0 or 1 for video/TV cards, and -1 for USB cameras Set to 0 for uvideo(4) on OpenBSD : " vdoipd
read -p "Rotate image this number of degrees. The rotation affects all saved images as well as movies. Valid values: 0 (default = no rotation), 90, 180 and 270. : " rt
read -p "Image width (pixels). Valid range: Camera dependent, default: 352 : " imwd
read -p "Image height (pixels). Valid range: Camera dependent, default: 288 : " imht
read -p "Maximum number of frames to be captured per second. Valid range: 2-100. Default: 100 (almost no limit). : " fps
read -p "Target dir folder where files will be saved. (deffault /var/www/html/recordings): " tgdir

#test if user pressed enter as a value
if [ ${#dmon} -eq 0 ]; then
  dmon="off"
  i="true"
elif [ ${#stupm} -eq 0 ]; then
  stupm="off"
  b="true"
elif [ ${#rt} -eq 0 ]; then
  rt="0"
  ...
  fps="100"
  i="true"
elif [ ${#tgdir} -eq 0 ]; then
  tgdir="/var/www/html/recordings"
  i="true"
fi

clear
echo "Creating config files from user input"
sleep 1
echo ""
echo "Please wait while the config file is being created."

#make custom config file using user input
echo "
...
##############################################################
#
; camera_dir /etc/motion/conf.d# Rename this distribution example file to motion.conf
#
# This config file was generated by motion 4.0.1
...
# Target base directory for pictures and films
# Recommended to use absolute path. (Default: current working directory)
target_dir /var/www/html/recordings
...
sudo /home/pi/Raspisecurity/scripts/raspisecurityinstaller.sh
